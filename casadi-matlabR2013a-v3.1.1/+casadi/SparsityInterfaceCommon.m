classdef SparsityInterfaceCommon < SwigRef
  ...
  methods
    ...
    %  {IM}} = BLOCKSPLIT(IM x, int vert_incr, int horz_incr)
    ...
    %  {IM}} = BLOCKSPLIT(IM x, [int] vert_offset, [int] horz_offset)
    ...
    %  {IM} = DIAGSPLIT(IM x, [int] output_offset)
    ...
    %  {IM} = DIAGSPLIT(IM x, int incr1, int incr2)
    %  {IM} = DIAGSPLIT(IM x, [int] output_offset1, [int] output_offset2)
    ...
    %  IM = RESHAPE(IM a, [int,int] rc)
    %  IM = RESHAPE(IM a, Sparsity sp)
    ...
    %  IM = RESHAPE(IM a, int nrow, int ncol)
    ...
    %  IM = TRIU(IM a, bool includeDiagonal)
    ...
    %  IM = TRIL(IM a, bool includeDiagonal)
    ...
    %  IM = REPMAT(IM A, int n, int m)
    %  IM = REPMAT(IM A, [int,int] rc)
