classdef Callback < casadi.Function
  ...
  methods
    ...
    %
      if nargin==1 && strcmp(class(varargin{1}),'SwigRef')
        ...
        if strcmp(class(self),'director_basic.Callback')
          tmp = casadiMEX(928, 0, varargin{:});
        else
          tmp = casadiMEX(928, self, varargin{:});
        end
    ...
    %
    %  Function = GET_JACOBIAN(self, char name, struct opts)
    %
    %
    %
    %
    ...
    %GET_FORWARD [DEPRECATED] Overload get_forward_new instead
    %
    %  Function = GET_FORWARD(self, char name, int nfwd, casadi::Dict & opts)
    %
    %
    %
    %
    ...
    function varargout = get_forward_new(self,varargin)
    %GET_FORWARD_NEW Return function that calculates forward derivatives forward(nfwd) returns a
    %
    %  Function = GET_FORWARD_NEW(self, char name, int nfwd, [char] i_names, [char] o_names, struct opts)
    %
    ...
    function varargout = get_n_forward(self,varargin)
    %GET_N_FORWARD Return function that calculates forward derivatives forward(nfwd) returns a
    %
    %  int = GET_N_FORWARD(self)
    %
    %cached instance if available, and calls  Function get_forward(int nfwd) if
    %no cached version is available.
    %
    ...
    %GET_REVERSE [DEPRECATED] Overload get_reverse_new instead
    %
    %  Function = GET_REVERSE(self, char name, int nadj, casadi::Dict & opts)
    %
    %
    %
    %
    ...
    function varargout = get_reverse_new(self,varargin)
    %GET_REVERSE_NEW Return function that calculates adjoint derivatives reverse(nadj) returns a
    %
    %  Function = GET_REVERSE_NEW(self, char name, int nadj, [char] i_names, [char] o_names, struct opts)
    %
    ...
    function varargout = get_n_reverse(self,varargin)
    %GET_N_REVERSE Return function that calculates adjoint derivatives reverse(nadj) returns a
    %
    %  int = GET_N_REVERSE(self)
    %
    %cached instance if available, and calls  Function get_reverse(int nadj) if
    %no cached version is available.
    %
  ...
  methods(Static)
    ...
    %CREATE 
    %
    %  Function = CREATE(char name, Callback n, struct opts)
    %
    %
