...
// apply a contactGeometry for Ground
	...
	// creates a mesh for a body:
		...
		if (CurBodyContProp->Geometry == 1 || CurBodyContProp->Geometry == 2)
		{
		...
		{
			int resolution = 6;
			PolygonalMesh BodyMesh0(PolygonalMesh::createBrickMesh(Vec3(CurBodyContProp->box_dim), resolution));

			printf("1.2. A box contact surface for body #%i is created! \n", i);
			BodyMesh = BodyMesh0;
			printf(" succeed! \n");
		}
		...
		// this code should be used once simbody bricks contact will be fully implemented. Currently simbody 3.5.1 doesn't handle brick-mesh contact.
		/*else if (CurBodyContProp->Geometry == 2) // box
		{
			Body.addContactSurface(Transform(Vec3(CurBodyContProp->Transform)),
