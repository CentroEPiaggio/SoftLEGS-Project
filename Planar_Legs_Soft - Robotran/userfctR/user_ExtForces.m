...
[F1,F2] = mbs_get_F_sensor_id(MBS_info,'ExtForce_Right','ExtForce_Left');
switch(ixF)
    case F1
        Fy = mbs_data.SFor{F1}(1); % instructions for case 1
        Fz = mbs_data.SFor{F1}(2);
        Mx = mbs_data.SFor{F1}(3);
    case F2
        Fy = mbs_data.SFor{F2}(1); % instructions for case 2
        Fz = mbs_data.SFor{F2}(2);
        Mx = mbs_data.SFor{F2}(3);
end
