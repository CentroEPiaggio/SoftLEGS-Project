...
save(name_sim,'index','res_K','h_foot','T','status','val','walk_dist','res_q','res_qd','res_qdd','res_ts','res_theta','res_thetad','tau','res_Fc','inv_err0','inv_err1');
...
% plot invariants
time_s = time(N_trans+1:end);
% figure(1);
% subplot(3,1,1);
inv_err_pos_x = inv_err1(1,N_trans+1:end);
% plot(time_s, inv_err_pos_x')
% legend('c_1(x)')

% subplot(3,1,2);
inv_err_pos_y = inv_err1(2,N_trans+1:end);
% plot(time_s, inv_err_pos_y')
% legend('c_1(y)')
% 
% subplot(3,1,3);
inv_err_pos_gamma = inv_err1(5,N_trans+1:end);
% plot(time_s, inv_err_pos_gamma')
% legend('alpha_1')
% xlabel('time (s)')
% title('invariants')
...
% % stiffness analysis
% [Kc_swing, eig_values_swing, eig_vect_swing, detJJT_swing] = Cartesian_Stiffness_left(MBS_data, res_q, res_K);
% [Kc_stance, eig_values_stance, eig_vect_stance, detJJT_stance] = Cartesian_Stiffness_right(MBS_data, res_q, res_K);
% 
