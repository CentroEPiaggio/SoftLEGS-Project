...
if flag_init
   ...
   str_n=['double n_traj = ',num2str(ceil(size(VIS_SIMS,2)/toSKip)),';'];
   fprintf(fileID,str_n);
   fprintf(fileID,'\n'); 
   
   for k=1:size(VIS_SIMS,2)
%    str_i=['Eigen::MatrixXd traj_',num2str(k),'(',num2str(size(VIS_SIMS{7,k},1)),',',num2str(size(VIS_SIMS{7,k},2)),');'];
...
%    str_pt=['Eigen::VectorXd *traj_p = new Eigen::VectorXd[',num2str(size(VIS_SIMS,2)),'];'];
   str_pt=['std::vector<Eigen::MatrixXd*> traj_M(',num2str(size(VIS_SIMS,2)),');\n'];
   fprintf(fileID,str_pt);
   fprintf(fileID,'\n'); 
   
   str_name=['std::vector<std::string> sim_name(',num2str(size(VIS_SIMS,2)),');'];
...
str_name_init=['sim_name[',num2str(n_traj-1),'] = ''',d{2},'''',';'];
   fprintf(fileID,str_name_init);
   fprintf(fileID,'\n'); 
str_pt=['traj_M.at(',num2str(n_traj-1),')=&traj_',num2str(n_traj),';'];
   fprintf(fileID,str_pt);
   fprintf(fileID,'\n\n'); 
...
%     for jj=1:THc
%         
% %     str=['traj_',num2str(n_traj),'(',num2str(i-1)',',',num2str(j-1),') = ',num2str(New_TH(i,j)),';\n'];
